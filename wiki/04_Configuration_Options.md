...
The `:options` table in `fplot` controls the overall appearance and
properties of your plot. It’s where you set titles, labels, ranges,
output files, and more.

Below is a comprehensive list of all available options. Remember that
when using Lua, keys with hyphens (like `:output-file`) must be written
using the `["key-name"] = value` syntax.
...
For options specific to individual datasets (like line color and style),
see the **Dataset Configuration** section.
